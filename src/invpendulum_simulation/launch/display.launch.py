...
def generate_launch_description():
    ...
    # Default RViz config (override with rvizconfig:=... when launching if needed)
    ...
    # Static TF: world -> base_link (so RViz can use "world" as Fixed Frame)
    static_tf = Node(
        package='tf2_ros',
    ...
    # JointState republisher: joint_states_raw -> joint_states (renames to URDF names)
    repub = Node(
        package='invpendulum_simulation',
        executable='joint_state_republisher.py',
        name='joint_state_republisher',
        output='screen',
        # remappings=[('joint_states_raw', 'joint_states')],
        parameters=[{'use_sim_time': use_sim_time}],
    )

    # RViz with your config
    rviz = Node(
        package='rviz2',
        executable='rviz2',
        name='rviz2',
    ...
    # One-shot seed so TF tree appears on first run (before MCU connects)
    seed_once = TimerAction(
        ...
        actions=[ExecuteProcess(
            cmd=[
                ...
                "{name: ['joint_base_to_arm','joint_arm_to_pendulum'], position: [0.0, 0.0]}"
            ],
            output='screen'
        )]
    )
